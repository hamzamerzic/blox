...
def test_sharded_linear_model_parallel():
  ...
  linear = bx.Linear(
    graph.child('linear'),
    output_size=16,
    w_metadata={'sharding': (None, 'model')},
...
def test_sharded_mlp_tensor_parallel():
  ...
  layer2 = bx.Linear(
    graph.child('layer2'),
    output_size=4,
    w_metadata={'sharding': ('model', None)},
    b_metadata={'sharding': ()},
  )
...
def test_init_with_fold_in_axes_produces_different_params():
  ...
  w_shards = params._data[('root', 'linear', 'w')].value.addressable_shards
  w_shard_data = [np.asarray(s.data) for s in w_shards]
  assert not np.allclose(
    w_shard_data[0], w_shard_data[1]
